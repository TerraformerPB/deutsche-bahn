...
| **D**enial of Service | Erschöpfung des Upstream-Kontingents (Blockierung durch DB/transport.rest) | Token-Bucket (40/min), Circuit-Breaker mit Backoff, Mindestabstand pro Fahrt, Board-Cache 60 s | umgesetzt |
| **D**enial of Service | Einzelner Client löst über beliebige Bahnhöfe/Fahrt-IDs viele Upstream-Abrufe aus und verdrängt den Poller | Nutzerausgelöste Abrufe nur für bekannte Fahrten und mit eigenem serverweitem Kontingent (`CLIENT_UPSTREAM_PER_MIN`, Standard 12/min); bei Erschöpfung HTTP 429 | umgesetzt |
